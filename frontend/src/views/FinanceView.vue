  ...
  <div class="screen-in fin-wrap">
    <div class="fin-inner">
      ...
      <template v-else-if="finance.data">
        <!-- Headline revenue answers: best day, best month, top source. -->
        <div class="fin-hero">
          <div class="fc-card fin-hero-tile">
            <div class="fin-hero-lbl">{{ t('fin_top_day') }}</div>
            <div class="fin-hero-val">{{ topDay ? formatDate(topDay.date, ui.locale) : '—' }}</div>
            <div v-if="topDay" class="fin-hero-note" dir="ltr">{{ fmt(topDay.amount) }}</div>
          </div>
          <div class="fc-card fin-hero-tile">
            <div class="fin-hero-lbl">{{ t('fin_top_month') }}</div>
            <div class="fin-hero-val">{{ topMonth ? monthLabel(topMonth.month) : '—' }}</div>
            <div v-if="topMonth" class="fin-hero-note" dir="ltr">{{ fmt(topMonth.amount) }}</div>
          </div>
          <div class="fc-card fin-hero-tile accent">
            <div class="fin-hero-lbl">{{ t('fin_top_source') }}</div>
            <div class="fin-hero-val">{{ topSource ? t(topSource.key) : '—' }}</div>
            <div v-if="topSource" class="fin-hero-note" dir="ltr">{{ fmt(topSource.amount) }}</div>
          </div>
        </div>

        <!-- KPI row -->
        <div class="fin-kpis two">
          <div class="fin-kpi">
        ...
        <div class="fc-card fin-card">
          ...
          <div v-for="(tx, i) in incomeTxns" :key="i" class="fin-txn">
          ...
          <div v-if="!incomeTxns.length" class="fin-empty">{{ t('pr_no_visits') }}</div>
        </div>
      </template>
    </div>
  </div>
...
// Revenue-only view: the ledger shows income rows, expenses stay off this page.
const incomeTxns = computed(() => (finance.data?.txns || []).filter((x) => x.io === 'in'));

const topDay = computed(() => finance.data?.top_revenue_day || null);
const topMonth = computed(() => finance.data?.top_revenue_month || null);
const topSource = computed(() => finance.data?.top_income_source || null);
...
function monthLabel(ym) {
  ...
  return new Intl.DateTimeFormat(ui.locale === 'ar' ? 'ar-SA' : 'en-US', {
...
.fin-hero { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; }
.fin-hero-tile { padding: 20px; }
.fin-hero-tile.accent { background: linear-gradient(140deg, #12A594, #0E8C7E); border-color: transparent; }
.fin-hero-lbl { font-size: 13px; font-weight: 700; color: var(--muted-2); }
.fin-hero-tile.accent .fin-hero-lbl { color: rgba(255,255,255,.9); }
.fin-hero-val { font-family: var(--font-head); font-weight: 800; font-size: clamp(20px, 2.6vw, 26px); color: var(--ink); margin-top: 8px; line-height: 1.15; }
.fin-hero-tile.accent .fin-hero-val { color: #fff; }
.fin-hero-note { font-size: 13.5px; color: var(--brand); font-weight: 800; margin-top: 6px; }
.fin-hero-tile.accent .fin-hero-note { color: #fff; }

/* KPI row */
.fin-kpis { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; }
.fin-kpis.two { grid-template-columns: repeat(2, 1fr); }
